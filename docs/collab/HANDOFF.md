...
|----|------|-------|------|-------|
| T054 | OCR Coverage Matrix + PDCA Training Loop Hardening. Patched 5 live workflows via n8n REST: (1) `XtaSg9pLDuPERtI8` added `OCR_COVERAGE_REGISTRY` route (`numberOutputs=7`) + coverage fallback view + fail-soft Sheets reads; (2) `up1n75qEhbsXswii` added coverage read call + two-pass selector wiring + response fields (`coverage_status`,`few_shot_count`,`retrieval_mode`) + direct-path few-shot integration + fail-soft retrieval HTTP; (3) `KW0QRXxRh9MjdPaY` hardened Telegram correction parser (strict grammar, allowlist, truncation, invalid-key rejection) and added parse diagnostics into km payload; (4) `ztJ8oCBHREUPPry6` fixed KM metadata source priority to `ocr_bills[0]` and forwarded canonical metadata fields to km-log; (5) `yCqvdl3vrHGgiBMt` added coverage/loop metrics (`coverage_by_status`, `critical_fill_rate_by_doc_type`, `first_pass_success_rate`, `re_correction_rate`, `audited_vs_accepted_split`) + stable-drop alert streak logic. Verification: unknown-vendor gate `needs_review` ✅ (exec `161862`), degraded OCR_EXAMPLES-read fault injection still returns main OCR response ✅ (exec `161879`), gg-data coverage endpoint returns expected statuses ✅ (exec `161896`), feedback receiver audited path ran ✅ (exec `161875`), nowThai sync ✅. Limits: physical `OCR_COVERAGE_REGISTRY` tab not readable in this env (gateway fallback default rows active); Nexgen happy-path target (`few_shot_count>0` + non-empty customer/address) not met on current sample run. | Codex | 2026-03-14 | PARTIAL PASS |
