...
async function runTests() {
  const tests = [
    {
      name: 'Encouragement comment triggers reply and LINE notify',
      ...
      assert.ok(result.line.enabled, 'LINE notification should be enabled');
      assert.strictEqual(result.meta.likeStatus, 'FB_LIKE_OK');
    }
  },
    {
      name: 'Complaint comment sends complaint reply and LINE alert',
      payload: createFbPayload({
    ...
    {
      name: 'Sticker comment routes through sticker flow',
      payload: createFbPayload({
        message: '',
      ...
      assert: (result) => {
        assert.strictEqual(result.route, 'fb_sticker');
        assert.strictEqual(result.sheetType, 'FB_STICKER');
        assert.ok(result.facebook.reply?.message.includes('Sticker'), 'Sticker reply text expected');
      }
    },
    {
      ...
      assert.ok(result.line.message.includes('หมวดเทศกาล: งานบุญ/งานบวช'));
      assert.strictEqual(result.meta.likeStatus, 'FB_LIKE_OK');
    }
  },
    {
      ...
      assert.ok(result.line.message.includes('งานศพ'));
      assert.strictEqual(result.meta.likeStatus, 'FB_LIKE_OK');
    }
  },
    {
      ...
      assert.ok(result.line.message.includes('งานแต่งงาน'));
      assert.strictEqual(result.meta.likeStatus, 'FB_LIKE_OK');
    }
  },
    {
      ...
      assert.ok(result.line.message.includes('วันเกิด'));
      assert.strictEqual(result.meta.likeStatus, 'FB_LIKE_OK');
    }
  }
  ];
