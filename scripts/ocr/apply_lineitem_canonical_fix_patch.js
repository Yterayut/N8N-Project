...
function backup(workflow) {
  ...
  const file = path.join(dir, `${WORKFLOW_ID}-lineitem-fix-${stamp}.json`);
  fs.writeFileSync(file, `${JSON.stringify(workflow, null, 2)}\n`, 'utf8');
  return file;
}

function main() {
  const row = JSON.parse(query(`select id,name,nodes,connections,activeVersionId from workflow_entity where id='${WORKFLOW_ID}' limit 1;`))[0];
  ...
  const backupFile = backup(wf);

  const node = wf.nodes.find((n) => n.name === 'Code (Normalize + Validate)');
  ...
  node.parameters.jsCode = [
    ...
    "    const unit_price = toNum(pick(row, ['unit_price','price_per_unit','price','ราคาต่อหน่วย','ราคาลิตร','ราคา/หน่วย'], 0));",
    "    const quantity = toNum(pick(row, ['quantity','qty','amount_qty','จำนวน','จำนวนลิตร','ปริมาณ'], 0));",
    ...
    "      if((toNum(d.quantity)>0 || toNum(d.unit_price)>0) && toNum(d.amount)<=0) errs.push({field:`list_detail[${idx}].amount`,severity:'warning',code:'line_amount_missing'});",
  ...
  const nodesPath = path.join(tmpDir, `${WORKFLOW_ID}.linefix.nodes.json`);
  const conPath = path.join(tmpDir, `${WORKFLOW_ID}.linefix.connections.json`);
  fs.writeFileSync(nodesPath, JSON.stringify(wf.nodes), 'utf8');
  fs.writeFileSync(conPath, JSON.stringify(wf.connections), 'utf8');

  const sqlPath = path.join(tmpDir, `${WORKFLOW_ID}.linefix.update.sql`);
  const sql = [
    '.timeout 10000',
